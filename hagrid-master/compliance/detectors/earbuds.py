...
class EarbudsDetector(BaseDetector):
    ...
    def _detect_in_hand(
        ...
        thr = max(0.62, float(self.confidence_threshold) + 0.12)
    ...
    def _detect_ear_roi(self, frame: np.ndarray) -> DetectorResult:
        ...
        if self._face_mesh is not None:
            ...
            if out.multi_face_landmarks:
                ...
                # Estimate 3D head pose (yaw, pitch, roll) and distance scale (D_eye)
                yaw, pitch, roll, d_eye, nose_x = self._estimate_head_pose(lm, w, h)

                # Evaluate left and right ears with distance-scaled ROI & occlusion suppression
                # Wearer turns left (yaw > 0.22): wearer's right ear (234) is occluded
                # Wearer turns right (yaw < -0.22): wearer's left ear (454) is occluded
                left_occluded = (yaw < -0.22)   # Head turned right -> left ear occluded
                right_occluded = (yaw > 0.22)   # Head turned left -> right ear occluded

                if not left_occluded:
                    s_left, k_left, b_left = self._analyze_ear_scaled(
                        lm, frame, w, h, self._MESH_LEFT, "left", d_eye, nose_x, yaw
                    )
                    if b_left is not None and s_left >= 0.42:
                        hits.append(("left", s_left, k_left, b_left))

                if not right_occluded:
                    s_right, k_right, b_right = self._analyze_ear_scaled(
                        lm, frame, w, h, self._MESH_RIGHT, "right", d_eye, nose_x, yaw
                    )
                    if b_right is not None and s_right >= 0.42:
            ...
            if not self.model_unavailable_logged:
                print("[compliance] EarbudsDetector: no FaceMesh or YOLO weights")
                self.model_unavailable_logged = True
            self._clear_boxes()
        ...
        # Dampen weak twin when one side is dominant
        if left_score > 0 and right_score > 0:
            if left_score - right_score > 0.18:
                right_score *= 0.35
            elif right_score - left_score > 0.18:
                left_score *= 0.35

        self._ema_left = 0.35 * self._ema_left + 0.65 * left_score
        self._ema_right = 0.35 * self._ema_right + 0.65 * right_score

        # Require minimum confidence threshold of 0.58 for real-time earbud detection to prevent false positives
        thr = max(0.58, float(self.confidence_threshold))
        left_raw = self._ema_left >= thr
        right_raw = self._ema_right >= thr

        # Disambiguate "Both" vs single side
        if left_raw and right_raw and min(self._ema_left, self._ema_right) < thr + 0.10:
    ...
    def _analyze_ear_scaled(
        ...
        box_w = max(28, int(0.55 * d_eye))
        box_h = max(34, int(1.25 * box_w))

        if side == "right":  # Wearer's Right Ear (appears on camera-left)
            offset_u = -(0.72 + 0.25 * max(0.0, -yaw))
            ear_cx = int(r_eye[0] + offset_u * d_eye * ux + 0.08 * d_eye * vx)
            ear_cy = int(r_eye[1] + offset_u * d_eye * uy + 0.08 * d_eye * vy)
            # Ensure crop stays strictly to the left of the right outer eye corner
            ear_cx = min(ear_cx, int(r_eye[0] - 15 - box_w // 2))
        else:                # Wearer's Left Ear (appears on camera-right)
            offset_u = +(0.72 + 0.25 * max(0.0, yaw))
            ear_cx = int(l_eye[0] + offset_u * d_eye * ux + 0.08 * d_eye * vx)
            ear_cy = int(l_eye[1] + offset_u * d_eye * uy + 0.08 * d_eye * vy)
            # Ensure crop stays strictly to the right of the left outer eye corner
            ear_cx = max(ear_cx, int(l_eye[0] + 15 + box_w // 2))
        ...
        # Absolute Safety Check: Reject if crop overlaps the outer eye corners or eyes
        if side == "right" and x2 >= int(r_eye[0]) - 5:
            return 0.0, "earbuds", None
        if side == "left" and x1 <= int(l_eye[0]) + 5:
            return 0.0, "earbuds", None
    ...
    def _score_bud_in_roi(self, roi: np.ndarray, side: str = "") -> Tuple[float, str]:
        ...
        # --- 1) Black / Dark Earbud & Bluetooth Earpiece Path ---
        black = (v_ch < 78) & (s_ch < 90)
        very_black = (v_ch < 48) & (s_ch < 80)
        black_ratio = float(np.count_nonzero(black)) / area
        core_ratio = float(np.count_nonzero(very_black)) / area

        # --- Beard / Sideburn Hair Texture Rejection ---
        # High spatial Laplacian variance across crop with diffuse dark hair indicates beard/sideburn hair
        lap_var = float(cv2.Laplacian(gray, cv2.CV_64F).var())
        if lap_var > 650.0 and core_ratio < 0.025 and black_ratio > 0.18:
            return 0.0, "earbuds"

        colorful = (s_ch > 75) & (v_ch > 50) & ((h_ch < 30) | ((h_ch > 12) & (h_ch < 45)))
        color_ratio = float(np.count_nonzero(colorful)) / area
        if color_ratio > 0.04 and core_ratio < 0.02:
            return 0.0, "earbuds"
        ...
        black_obj = ((black & (dark_c > 10)) | very_black) & (~colorful)
        ...
        if contours:
            ...
            for c in ranked[:5]:
                blob = float(cv2.contourArea(c))
                blob_ratio = blob / area
                if blob_ratio < 0.025 or blob_ratio > 0.38:
                    continue
                bx, by, bw, bh = cv2.boundingRect(c)
                if min(bw, bh) < 5:
                    continue
                ...
                # Beard hair extending to bottom/side crop edge
                touches_bottom = (by + bh) >= rh - 2
                if touches_bottom and blob_ratio > 0.15 and aspect > 2.0:
                    continue

                # Lobe stud earring check
                lobe_stud = blob_ratio < 0.035 and cy > rh * 0.62 and circ > 0.42 and aspect < 1.8
                if lobe_stud:
                    continue
                best = c
                break

            if best is None and black_ratio > 0.55 and core_ratio > 0.30:
                return 0.0, "earbuds"
            ...
            if best is not None:
                ...
                # Skin ring check distinguishes empty ear canal from solid earbud
                skin_ring = self._skin_ring_ratio(blob_mask, v_ch, s_ch)
                canal_like = (
                    blob_ratio < 0.070
                    and circ > 0.28
                    and aspect < 1.85
                ...
                # Stem structure detection along lower ROI / jawline
                stem_bonus = 0.0
                sy1, sy2 = min(rh, by + bh), min(rh, by + bh + max(10, int(bh * 0.8)))
                ...
                if sy2 > sy1 and sx2 > sx1:
                    ...
                    if 0.08 <= sr <= 0.70:
                        col = np.mean(stem_dark.astype(np.float32), axis=0)
                        if float(np.max(col)) > 0.22:
                            stem_bonus = 0.35

                if canal_like and stem_bonus < 0.20:
                    return 0.0, "earbuds"

                # Without a stem bonus or very dark core, reject diffuse beard patches
                if stem_bonus < 0.20 and core_ratio < 0.03 and blob_ratio > 0.08:
                    return 0.0, "earbuds"

                if sat_mean <= 70 and v_mean < 100:
                    dark_bonus = 0.16 if v_mean < 50 else (0.08 if v_mean < 75 else 0.0)
                    contrast_mean = float(np.mean(np.maximum(dark_c, 0)[blob_mask > 0]))
                    black_score = (
                        0.25 * min(1.0, max(core_ratio, black_ratio * 0.35) / 0.045)
                        + 0.25 * min(1.0, blob_ratio / 0.07)
                        + 0.12 * min(1.0, max(contrast_mean, 1.0) / 24.0)
                        + 0.08 * min(1.0, max(circ, 0.10) / 0.40)
                        + stem_bonus
                        + dark_bonus
                        + (0.06 if sat_mean < 40 else 0.0)
                        + (0.06 if 1.2 <= aspect <= 2.8 else 0.0)
                    )
                    if skin_ring > 0.52 and stem_bonus < 0.18 and core_ratio < 0.035:
                        black_score *= 0.45
                    elif stem_bonus >= 0.18 or core_ratio >= 0.04:
                        black_score = min(1.0, black_score + 0.08)

                    # Over-ear Bluetooth headset / ear-hook signature check
                    if aspect > 2.2 and blob_ratio > 0.12:
                        kind_b = "bluetooth"

        # --- 2) White / Light Earbud (AirPods) Path ---
        white_score = 0.0
        kind_w = "earbuds"
        light = (v_ch > 165) & (s_ch < 55) & (~colorful)
        light_obj = light & ((bright_c > 7) | (v_ch > 215))
        if float(np.count_nonzero(light_obj)) / area >= 0.022:
            ...
            if lcnt:
                ...
                if 0.025 <= lblob <= 0.38 and laspect <= 3.4:
                    white_score = 0.42 * min(1.0, lblob / 0.08) + 0.30
