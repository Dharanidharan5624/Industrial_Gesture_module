...
class CameraScanWorker(QtCore.QThread):
    """Runs list_available_cameras() off the GUI thread.

    Probing each camera index (open + read + release) can take real time —
    on some machines/backends, several seconds per index — and the previous
    code called list_available_cameras() directly from the GUI thread,
    including once during MainWindow.__init__ before the window was even
    shown. That blocked the whole event loop for the full scan duration,
    which is exactly what the OS's "app is not responding" dialog detects.
    Running the scan here and reporting back via a signal keeps the UI
    responsive throughout.
    """

    scan_done = Signal(list)  # list[int] of available camera indices

    def __init__(self, skip_index: Optional[int] = None, parent=None):
        super().__init__(parent)
        self.skip_index = skip_index

    def run(self) -> None:
        try:
            available = list_available_cameras(skip_index=self.skip_index)
        except Exception:
            available = []
        self.scan_done.emit(available)
...
class CameraWorker(QtCore.QThread):
    ...
    def __init__(
        ...
        detection_mode: str = "screw_monitor",
        turn_target: float = 2.5,
        align_threshold_px: int = 50,
    ):
        super().__init__(parent)
        ...
        self.detection_mode = detection_mode   # 'screw_monitor' | 'stub'
        ...
        # whether the GUI thread has finished painting the previous frame.
        # Qt queues each emit as a separate cross-thread event, so if
        # detection+painting (gesture + monitor + compliance checks) is even
        # slightly slower than capture, frames pile up in the queue and the
        # on-screen video drifts further behind real time the longer the app
        # runs. Skipping the emit when a frame is still pending caps the
        # delay at roughly one frame instead of letting it grow.
        self._frame_pending = False
        self._state_pending = False

    def mark_frame_consumed(self) -> None:
        """Call from the UI thread's frame_ready slot once it has finished
        using the frame, so the worker knows it's safe to emit the next one."""
        self._frame_pending = False

    def mark_state_consumed(self) -> None:
    ...
    def run(self) -> None:
        ...
        # Only create ScrewMonitor for the rotation-count pipeline.
        # Stub assemblies skip it entirely so no invalid state accumulates.
        if self.detection_mode == "screw_monitor":
            self._monitor = ScrewMonitor(
                worker_id=self.worker_id,
                turn_target=self.turn_target,
                align_threshold=self.align_threshold_px,
    ...
    def update_assembly(self, assembly_id: str, detection_mode: str,
        ...
        """Update assembly context on the worker (call before restart)."""
        self.assembly_id = assembly_id
